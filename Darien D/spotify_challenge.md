...
<!-- 1.Creating a table for genre Give each genre a unique identifier (primary key) 2.Bring genre primary key to songs table as a foreign key -->
b) What relationship best describes how genre relates to songs in your answer to Question 1a?
<!-- 1. Genre and Songs have a one(genre) to many(songs) relationship -->
# Question 2

What changes would you make to the modeling of genre data if we could assign more than one genre to a song?
We had a one-to-many relationship between genres and songs. Now that if we are to assign multiple genres to one song, then we have many-to-many relationship between songs and genres. So the practice is to create a intermdiate table that maps the primary keys in the songs table to the primary keys in the genres table, just like what it is in the ArtistsMembers table.