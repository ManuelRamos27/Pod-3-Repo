...
class SubwayStation(Station):
    def __init__(self,station_name,location, lines):
        ...
        self.lines=lines
    def show_info(self):
     print(f"{self.station_name} station is located at {self.location} and the lines connected are {self.lines}")
class Busstation(Station):
    ...
    def show_info(self):
        if self.open ==True:
            ...
            self.open= 'Closed'
        print(f"{self.station_name} station is located at {self.location} and the routes are {self.routes} it is also {self.open}")
...
lines: ['1', '2', '3', 'L']
'''
Fourteenth_st=SubwayStation('14th street','14th street and 7th avenue',['1', '2', '3', 'L'])
...
'''
NYCbus= Busstation('NYC Megabus Stop','34th street and 12th avenue',['Boston', 'DC', 'Philly'])
NYCbus.show_info()
